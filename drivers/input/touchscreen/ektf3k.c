...
int elan_iap_open(struct inode *inode, struct file *filp)
{
	...
	if (private_ts == NULL)
		touch_debug(DEBUG_ERROR, "private_ts is NULL~~~");
			      ...
			      loff_t * offp)
{
	...
	if (ret != count)
		touch_debug(DEBUG_ERROR, "ELAN i2c_master_send fail, ret=%d \n",
			    ret);
	kfree(tmp);
	return ret;
...
static ssize_t elan_show_status(struct device *dev,
...
static int check_fw_version(const unsigned char *firmware, unsigned int size,
			    int fw_version)
{
	int id, version;
			       ...
			       const char *buf, size_t count)
{
	...
	firmware_fp->f_pos = 0;
	for (pos = 0, page_number = 0; pos < MAX_FIRMWARE_SIZE;
	     pos += FIRMWARE_PAGE_SIZE, page_number++) {
		if (firmware_fp->f_op->
		    read(firmware_fp, firmware + pos, FIRMWARE_PAGE_SIZE,
			 &firmware_fp->f_pos) != FIRMWARE_PAGE_SIZE) {
			break;
		}
	}
	...
	if (RECOVERY || check_fw_version(firmware, pos, ts->fw_ver) > 0) {
		...
		do {
			ret =
			    firmware_update_header(client, firmware,
						   page_number);
...
static int elan_ktf3k_touch_sysfs_init(void)
{
	...
	if (android_touch_kobj == NULL) {
		touch_debug(DEBUG_ERROR,
			    "[elan]%s: subsystem_register failed\n", __func__);
		ret = -ENOMEM;
		return ret;
	}
	...
	if (ret) {
		touch_debug(DEBUG_ERROR,
			    "[elan]%s: sysfs_create_group failed\n", __func__);
		return ret;
	}
	return 0;
}
...
static int elan_ktf3k_ts_read_command(struct i2c_client *client,
				      u8 * cmd, u16 cmd_length, u8 * value,
				      u16 value_length)
{
...
static int __hello_packet_handler(struct i2c_client *client)
{
	...
	touch_debug(DEBUG_INFO, "[elan] %s: hello packet %2x:%2X:%2x:%2x\n",
		    ...
		    buf_recv[3]);
	if (!
	    (buf_recv[0] == 0x55 && buf_recv[1] == 0x55 && buf_recv[2] == 0x55
	     && buf_recv[3] == 0x55)) {
		RECOVERY = 0x80;
		return RECOVERY;
	}
...
static int __fw_packet_handler(struct i2c_client *client, int immediate)
{
	...
	if (immediate) {
		...
		major =
		    ((buf_recv[1] & 0x0f) << 4) | ((buf_recv[2] & 0xf0) >> 4);
		minor =
		    ((buf_recv[2] & 0x0f) << 4) | ((buf_recv[3] & 0xf0) >> 4);
...
static int elan_ktf3k_ts_recv_data(struct i2c_client *client, uint8_t * buf,
				   int size)
{
...
static void elan_ktf3k_ts_report_data(struct i2c_client *client, uint8_t * buf)
{
	...
	if ((num < 3) || ((checksum & 0x00ff) == buf[34])) {
		...
		for (i = 0; i < FINGER_NUM; i++) {
			...
			if (active || mTouchStatus[i]) {
				...
				input_mt_report_slot_state(ts->input_dev,
							   ...
							   active);
				if (active) {
					elan_ktf3k_ts_parse_xy(&buf[idx], &x,
							       &y);
					x = x >
					    ts->abs_x_max ? 0 : ts->abs_x_max -
					    x;
					y = y >
					    ts->abs_y_max ? ts->abs_y_max : y;
					touch_size =
					    ((i & 0x01) ? buf[size_index[i]]
					     : (buf[size_index[i]] >> 4)) &
					    0x0F;
					...
					input_report_abs(idev,
							 ABS_MT_TOUCH_MAJOR,
							 touch_size);
					input_report_abs(idev, ABS_MT_PRESSURE,
							 pressure_size);
					input_report_abs(idev,
							 ABS_MT_POSITION_X, x);
					input_report_abs(idev,
		...
		checksum_err += 1;
		touch_debug(DEBUG_ERROR,
			    "[elan] Checksum Error %d byte[2]=%X\n",
			    checksum_err, buf[2]);
	}
...
static void process_resp_message(struct elan_ktf3k_ts_data *ts,
				 const unsigned char *buf, unsigned int size)
{
	...
	default:
		touch_debug(DEBUG_INFO,
			    "[elan] Get unknow packet {0x%02X, 0x%02X, 0x%02X, 0x%02X}\n",
...
static void elan_ktf3k_ts_work_func(struct work_struct *work)
{
	...
	if (work_lock != 0) {
		touch_debug(DEBUG_INFO,
			    "Firmware update during touch event handling");
	...
	case NORMAL_PKT:
		...
		// Second package
		if ((buf[1] == 2) || (buf[1] == 3)) {
			rc = elan_ktf3k_ts_recv_data(ts->client, buf1,
						     PACKET_SIZE);
			if (rc < 0) {
				enable_irq(ts->client->irq);
				return;
		...
		// Final package
		if (buf[1] == 3) {
			rc = elan_ktf3k_ts_recv_data(ts->client, buf2,
						     PACKET_SIZE);
			if (rc < 0) {
				enable_irq(ts->client->irq);
				return;
		...
		break;
	case NEW_NOMARL_PKT:
		rc = elan_ktf3k_ts_recv_data(ts->client, buf + 4,
					     NEW_PACKET_SIZE); // read the finger report packet.
		up(&pSem);
...
#ifdef _ENABLE_DBG_LEVEL
static int ektf_proc_read(char *buffer, char **buffer_location, off_t offset,
...
static int ektf_proc_write(struct file *file, const char *buffer,
			   unsigned long count, void *data)
{
	...
	if (copy_from_user(procfs_buf, buffer, procfs_buffer_size)) {
		touch_debug(DEBUG_ERROR,
			    " proc_write faied at copy_from_user\n");
		return -EFAULT;
	}
...
static int sendI2CPacket(struct i2c_client *client, const unsigned char *buf,
			 unsigned int length)
{
	int ret, i, retry_times = 10;
	for (i = 0; i < length; i += ret) {
		ret =
		    i2c_master_send(client, buf + i,
				    length < SIZE_PER_PACKET ?
				    length : SIZE_PER_PACKET);
		if (ret <= 0) {
			retry_times--;
			ret = 0;
		...
		if (retry_times < 0) {
			touch_debug
			    ("Failed sending I2C touch firmware packet.\n");
			break;
		}
	}
...
static int recvI2CPacket(struct i2c_client *client, unsigned char *buf,
			 unsigned int length)
{
	int ret, i, retry_times = 10;
	for (i = 0; i < length; i += ret) {
				  ...
				  unsigned int pages_number)
{
	...
	if (boot_code == 0) {	// if the boot code is old
		...
		touch_debug(DEBUG_INFO,
			    "The received bytes 0x%X 0x%X 0x%X 0x%X\n",
		...
		touch_debug(DEBUG_INFO,
			    "The firmware update of new boot code\n");
...
page_write_retry:
		...
		int recvCount;
		if ((recvCount =
		     recvI2CPacket(client, packet_data,
				   FIRMWARE_ACK_SIZE)) != FIRMWARE_ACK_SIZE) {
			dev_err(&client->dev, "Fail to Update page number %d\n",
				i);
			goto fw_update_failed;
		}
		...
		if (packet_data[0] != 0xaa || packet_data[1] != 0xaa) {
			touch_debug(DEBUG_INFO,
				    "message received: %02X %02X Page %d rewrite\n",
...
int elan_stress_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case STRESS_POLL_DATA:
		if (arg == STRESS_IOCTL_START_HEAVY) {
			...
			queue_delayed_work(stress_work_queue,
					   &elan_poll_data_work, poll_mode);
			       ...
			       const struct i2c_device_id *id)
{
	...
	if (ts == NULL) {
		touch_debug(DEBUG_INFO,
			    "[elan] %s: allocate elan_ktf3k_ts_data failed\n",
			    __func__);
		err = -ENOMEM;
...
#ifdef FIRMWARE_UPDATE_WITH_HEADER
	if (RECOVERY
	    || check_fw_version(touch_firmware, sizeof(touch_firmware),
				ts->fw_ver) > 0)
		firmware_update_header(client, touch_firmware,
				       sizeof(touch_firmware) /
				       FIRMWARE_PAGE_SIZE);
#endif
	...
	if (!stress_work_queue) {
		dev_err(&client->dev,
			"Unable to create stress_work_queue workqueue\n");
	}
	...
	if (err) {
		dev_err(&client->dev,
			"elan stress test: Unable to register %s \\misc device\n",
			ts->misc_dev.name);
	}
#endif
	...
	if (switch_dev_register(&ts->touch_sdev) < 0) {
		touch_debug(DEBUG_ERROR,
			    "switch_dev_register for dock failed!\n");
		//goto exit;
	}
...
static int elan_ktf3k_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (work_lock == 0)
		rc = elan_ktf3k_ts_set_power_state(client,
